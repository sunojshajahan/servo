...
# =========================================================
# ROI / LINE SETTINGS (<<< ВАЖНО ДЛЯ ТВОЕГО ВОПРОСА >>>)
# =========================================================
ROI_HEIGHT_FRACTION = 0.40     # высота ROI как доля кадра
ROI_Y_OFFSET_FRAC   = -0.10    # отрицательное = поднять ROI вверх (например -0.20), положительное = вниз

# положение линии обработки внутри ROI:
# 0.0 = верх ROI, 0.5 = центр ROI, 1.0 = низ ROI
LINE_Y_IN_ROI_FRAC  = 0.50
...
# RUN profile
SERVO_UPDATE_EVERY_N_FRAMES_RUN = 2
SERVO_SMOOTH_ALPHA_RUN = 0.25
SERVO_MAX_STEP_DEG_RUN = 10.0

# deadzone around frame center (pixels) to reduce jitter
SERVO_DEADZONE_PX = 6

# CALIB profile
SERVO_UPDATE_EVERY_N_FRAMES_CALIB = 1
SERVO_SMOOTH_ALPHA_CALIB = 0.60
SERVO_MAX_STEP_DEG_CALIB = 60.0

# manual step sizes in calibration
...
# по умолчанию инвертируем (у тебя была обратная реакция)
SERVO_INVERT_X = True

SERVO_OFFSET_DEG = 0.0
SERVO_GAIN = 1.00

# linear model: angle = k*cx + b
CAL_K = 0.0
...
def map_cx_to_angle(cx, w):
    """
    Mapping cx -> angle (учёт калибровки)
    """
...
def compute_roi(h):
    """
    ROI с вертикальным сдвигом:
    ROI_Y_OFFSET_FRAC < 0 -> вверх
    """
    roi_h = int(h * ROI_HEIGHT_FRACTION)
    roi_h = max(10, min(h, roi_h))

    center_y = h // 2
    shift_px = int(h * ROI_Y_OFFSET_FRAC)
    roi_center_y = center_y + shift_px

    y1 = int(roi_center_y - roi_h / 2)
    y1 = max(0, min(h - roi_h, y1))
    y2 = y1 + roi_h
    return y1, y2, roi_h
    ...
    with dai.Pipeline() as pipeline:
        ...
        try:
            while pipeline.isRunning():
                ...
                # ROI position (with vertical offset)
                y1, y2, roi_h = compute_roi(h)
                roi = frame[y1:y2, :]

                # line position inside ROI
                mid_y = int(y1 + LINE_Y_IN_ROI_FRAC * (y2 - y1))
                ...
                if results and len(results) > 0 and results[0].boxes is not None:
                    ...
                    if ids is not None and xyxy is not None:
                        ...
                        for i, (bx1, by1, bx2, by2) in enumerate(xyxy):
                            ...
                            # coords are relative to ROI, so shift Y by y1
                            ...
                            if current_target_id is not None:
                                ...
                                # candidates only from below the line (expected movement bottom->top)
                                if cy < mid_y:
                                    continue
                    ...
                    if calib_enabled:
                        ...
                        if frame_idx % SERVO_UPDATE_EVERY_N_FRAMES_RUN == 0:
                            if current_target_id and target_found and target_cxcy is not None:
                                ...
                                # deadzone around image center (optional)
                                ...
                                if PRINT_TARGET:
                                    print(f"[TARGET] cx={cx} -> angle={new_angle:.2f}")
                ...
                if DEBUG_SHOW:
                    ...
                    lines = [
                        ...
                        f"ROI: Hfrac={ROI_HEIGHT_FRACTION:.2f}  Yoff={ROI_Y_OFFSET_FRAC:+.2f}  LineInROI={LINE_Y_IN_ROI_FRAC:.2f}",
                    ...
                    cv2.imshow("OAK + YOLO + Servo (FINAL)", output)
                    ...
                    # manual aim (only useful in calib mode)
                    if key == ord("a"):
