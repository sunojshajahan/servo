...
# BOARD numbering (physical pin numbers on 40-pin header)
# IMPORTANT: choose a pin that supports PWM on your Jetson configuration.
SERVO_PIN_BOARD = 32

SERVO_FREQ_HZ = 50  # standard servo = 50Hz (20ms period)

# Calibrate pulse range for your servo (typical values)
SERVO_MIN_PULSE_MS = 0.5   # 0.5ms
SERVO_MAX_PULSE_MS = 2.5   # 2.5ms

# Servo angle limits you want to use
SERVO_MIN_ANGLE = 0
SERVO_MAX_ANGLE = 180

# How often to update servo based on latest target
SERVO_UPDATE_EVERY_N_FRAMES = 2

# Smooth servo (helps reduce jitter)
SERVO_SMOOTH_ALPHA = 0.25     # 0..1 (higher = faster response)
SERVO_MAX_STEP_DEG = 10.0     # limit per update to avoid sharp jumps

# If no target found: keep last angle or return to center
SERVO_ON_NO_TARGET = "hold"   # "hold" or "center"
...
if not os.environ.get("DISPLAY"):
    # If running via SSH without X forwarding, OpenCV windows will crash
    DEBUG_SHOW = False
...
class ServoController:
    ...
    def start(self):
        ...
        # Use PWM (hardware if configured, otherwise software PWM)
        self._pwm = GPIO.PWM(self.pin, self.freq)
        self._pwm.start(0.0)  # start with 0 duty (no signal)
        time.sleep(0.05)
...
def map_cx_to_angle(cx: int, w: int) -> float:
    # cx in [0..w] -> angle in [SERVO_MIN_ANGLE..SERVO_MAX_ANGLE]
    if w <= 1:
...
def main():
    ...
    try:
        while True:
            ...
            # =========================
            # SERVO UPDATE (use cx only)
            # =========================
            if SERVO_ENABLED and servo is not None:
                if frame_idx % SERVO_UPDATE_EVERY_N_FRAMES == 0:
            ...
            if trigger_ids:
                ...
                if current_target_id in trigger_ids:
                    # Здесь можно добавить действие (например включить нож/реле/второй мотор)
                    # Сейчас просто сброс цели:
                    current_target_id = None
                    target_missing_frames = 0

            # показ / выход
            if DEBUG_SHOW:
                cv2.imshow("Weed tracking (Direct Servo, no Arduino)", output)
