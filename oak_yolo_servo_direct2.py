...
SERVO_PIN_BOARD = 15        # physical pin 15 (BOARD numbering)
...
SERVO_SMOOTH_ALPHA = 0.75
SERVO_MAX_STEP_DEG = 20.0
...
class ServoController:
    ...
    def start(self):
        ...
        self._pwm.start(0.0)  # start with 0 duty
        time.sleep(0.05)
...
def get_bgr_frame(img_frame: dai.ImgFrame):
    """
    Надёжно получить BGR кадр как numpy array.
    Обычно getCvFrame() работает, но оставим fallback.
    """
    try:
        return img_frame.getCvFrame()
    except Exception:
        # fallback: raw frame -> reshape -> BGR
        data = img_frame.getFrame()
        h = img_frame.getHeight()
        w = img_frame.getWidth()
        # BGR888p = 3 bytes per pixel
...
def main():
    ...
    with dai.Pipeline() as pipeline:
        ...
        # ВАЖНО: resizeMode (camelCase), а не resize_mode
        ...
        try:
            while pipeline.isRunning():
                ...
                frame = get_bgr_frame(video_in)  # BGR numpy
